...
async function main() {
  ...
  console.log(`Excel: ${excelRows.length} products`);

  // 2. Leer DB
  const pool = await getPool();
  const dbResult = await pool.request().query(`
    SELECT id, cmhReference, panel, col, [row] FROM Products ORDER BY id
  `);
    ...
    if (dbPanel !== expected.panel || dbCol !== expected.col || dbRow !== expected.row) {
      ...
      try {
        ...
        const ocupado = await pool.request()
          ...
          .query(`SELECT id, cmhReference FROM Products WHERE panel = @panel AND col = @col AND [row] = @row AND id != @excludeId`);

        if (ocupado.recordset.length > 0) {
          const ocupante = ocupado.recordset[0];
          console.log(`    ⚠ Posición ocupada por ${ocupante.cmhReference} (ID ${ocupante.id}), se intercambian`);
          ...
          try {
            ...
            await transaction.request()
              ...
              .query(`UPDATE Products SET panel = @p1, col = @c1, [row] = @r1b, updatedAt = SYSUTCDATETIME() WHERE id = @id2`);
            ...
            await transaction.request()
              ...
              .query(`UPDATE Products SET panel = @p2, col = @c2, [row] = @r2b, updatedAt = SYSUTCDATETIME() WHERE id = @id1`);
  ...
  console.log(`\nResumen: ${fixed} corregidos, ${errors} errors, ${excelRows.length - fixed - errors} ya correctos`);
  await pool.close();
}
