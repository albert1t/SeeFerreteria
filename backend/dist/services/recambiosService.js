...
export async function getPreview(incluirOcultos = false) {
    return recambiosRepo.findPreview(incluirOcultos);
}
export async function getRecambio(id) {
    const recambio = await recambiosRepo.findById(id);
    if (!recambio)
        throw new AppError(404, 'Recambio no encontrado');
    return recambio;
}
export async function getRecambioByRef(ref) {
    const recambio = await recambiosRepo.findByReferencia(ref);
    if (!recambio)
        throw new AppError(404, 'Referencia no encontrada');
    return recambio;
}
async function validateUbicacion(panel, col, row, excludeId) {
...
export async function createRecambio(data, skipDupeCheck = false) {
    if (!skipDupeCheck) {
...
export async function updateRecambio(id, data) {
...
export async function deleteRecambio(id) {
...
export async function getCubetasPanel(panel, incluirOcultos = false) {
...
export async function swapPositions(id1, id2) {
    ...
    if (!r1)
        throw new AppError(404, `Recambio ${id1} no encontrado`);
    if (!r2)
        throw new AppError(404, `Recambio ${id2} no encontrado`);
...
export async function importarDesdeExcel(buffer) {
    ...
    const allRefs = allRows
        ...
        .filter(Boolean)
        .map(String);
    const existingRefs = allRefs.length > 0 ? await recambiosRepo.findExistingReferencias(allRefs) : new Set();
    let total = 0;
    let insertados = 0;
    const errores = [];
    for (const { sheetName, row } of allRows) {
        ...
        try {
            ...
            if (existingRefs.has(String(referenciaCMH))) {
                total--;
                continue;
            }
