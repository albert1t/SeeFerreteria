...
async function verifyMsalToken(idToken) {
    if (!env.AZURE_AD_CLIENT_ID) {
        throw new AppError(500, 'Azure AD no está configurado en el servidor');
    }
    const unverified = JSON.parse(Buffer.from(idToken.split('.')[1], 'base64url').toString());
...
export async function loginMicrosoft(idToken) {
    ...
    const username = (payload.email || payload.preferred_username);
    if (!username) {
