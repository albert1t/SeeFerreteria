...
export async function createProduct(data, skipDupeCheck = false) {
    if (!skipDupeCheck) {
...
export async function swapPositions(id1, id2) {
    ...
    if (!r1)
        throw new AppError(404, `Product ${id1} no encontrado`);
    if (!r2)
        throw new AppError(404, `Product ${id2} no encontrado`);
...
export async function importFromExcel(buffer) {
    ...
    const allRefs = allRows
        ...
        .filter(Boolean)
        .map(String);
    const existingRefs = allRefs.length > 0 ? await productsRepo.findExistingReferencias(allRefs) : new Set();
    let total = 0;
    let insertados = 0;
    const errors = [];
    for (const { sheetName, row } of allRows) {
        ...
        try {
            ...
            if (existingRefs.has(String(cmhReference))) {
                total--;
                continue;
            }
