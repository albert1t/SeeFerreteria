...
router.get('/importar/:marca/estado', validateParams(importarMarcaSchema), async (req, res, next) => {
    try {
...
router.post('/importar/:marca', requirePermission('tarifas', 'edit'), validateParams(importarMarcaSchema), (req, res, next) => {
    ...
    try {
        ...
        const resultado = await importacionesService.importarCsv(marca, req.file.buffer, req.file.originalname, req.user.userId);
        res.status(201).json({
            ok: true,
