...
import { productCreateSchema, productUpdateSchema, productsQuerySchema } from '../schemas/index.js';
 ...
 */
router.post('/upload-image', requireRole('admin'), (req, res, next) => {
    try {
    ...
    try {
        if (!req.file) {
            res.status(400).json({ error: 'No se ha enviado ninguna image' });
            return;
        }
        const file = req.file;
        const sasUrl = env.AZURE_BLOB_SAS_URL;
        const ext = file.originalname.split('.').pop() ?? 'jpg';
        const blobName = `product-image/product-${Date.now()}-${Math.round(Math.random() * 1e9)}.${ext}`;
...
router.get('/', validateQuery(productsQuerySchema), async (req, res, next) => {
    try {
        const q = req.query;
        if (q.preview === 'true') {
            const items = await productsService.getPreview(q.incluirOcultos === 'true');
            res.json(items);
            return;
        }
...
router.get('/ref/:ref', async (req, res, next) => {
    try {
        ...
        const product = await productsService.getRecambioByRef(ref);
        res.json(product);
    }
    catch (err) {
        next(err);
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        const product = await productsService.getRecambio(id);
        const orders = await ordersService.getOrdersByProduct(id);
...
router.post('/import', requireRole('admin'), uploadExcel.single('file'), async (req, res, next) => {
    try {
        ...
        const result = await productsService.importFromExcel(req.file.buffer);
        res.json(result);
    }
    catch (err) {
        next(err);
...
router.post('/', requireRole('admin'), validateBody(productCreateSchema), async (req, res, next) => {
    try {
...
router.patch('/:id/hidden', requireRole('admin'), async (req, res, next) => {
    try {
        const id = parseInt(String(req.params.id), 10);
        const product = await productsService.toggleOculto(id);
        res.json(product);
    }
    catch (err) {
        next(err);
...
router.post('/swap', requireRole('admin'), async (req, res, next) => {
    try {
        ...
        const result = await productsService.swapPositions(Number(id1), Number(id2));
        res.json(result);
    }
    catch (err) {
        next(err);
