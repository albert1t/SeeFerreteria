...
function mapRecambio(row) {
    return {
        id: row.id,
        referenciaCMH: row.referenciaCMH,
...
export async function findExistingReferencias(refs) {
    ...
    const [rows] = await pool.query(`SELECT DISTINCT referenciaCMH FROM Recambios WHERE referenciaCMH IN (${placeholders})`, refs);
    return new Set(rows.map((row) => row.referenciaCMH));
}
...
export async function create(data) {
    const pool = await getPool();
    const [result] = await pool.query(`INSERT INTO Recambios (referenciaCMH, referenciaCliente, codigo, nombre, marca, descripcion,
      metrica, unidadEmbalaje, imagen, plazoEntrega, familiaId, nReposicion, panel, col, row, oculto)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [data.referenciaCMH, data.referenciaCliente, data.codigo, data.nombre, data.marca,
    ...
    if (!created)
        throw new Error('Failed to create recambio');
    return created;
}
export async function update(id, data) {
    ...
    if (!existing)
        return null;
    await pool.query(`UPDATE Recambios SET
    ...
    WHERE id = ?`, [data.referenciaCMH ?? existing.referenciaCMH, data.referenciaCliente ?? existing.referenciaCliente,
...
export async function setOculto(id, oculto) {
    const pool = await getPool();
...
export async function getPanelResumen() {
  ...
  `);
    return rows.map((r) => ({
        panel: r.panel,
        totalRecambios: r.totalRecambios,
    }));
}
...
export async function swapPositions(id1, id2) {
    ...
    if (!r1 || !r2)
        throw new Error('Recambio no encontrado');
    const connection = await pool.getConnection();
    await connection.beginTransaction();
    try {
        await connection.query("UPDATE Recambios SET panel = 'ZZ', col = 1, `row` = ?, updatedAt = UTC_TIMESTAMP(6) WHERE id = ?", [id1 % 15 + 1, id1]);
        await connection.query('UPDATE Recambios SET panel = ?, col = ?, `row` = ?, updatedAt = UTC_TIMESTAMP(6) WHERE id = ?', [r1.panel, r1.col, r1.row, id2]);
        await connection.query('UPDATE Recambios SET panel = ?, col = ?, `row` = ?, updatedAt = UTC_TIMESTAMP(6) WHERE id = ?', [r2.panel, r2.col, r2.row, id1]);
...
export async function getPanelOccupancy(panel) {
    const pool = await getPool();
    const [rows] = await pool.query('SELECT col, `row`, id AS recambioId FROM Recambios WHERE panel = ?', [panel]);
    return rows.map((r) => ({
        col: r.col,
        row: r.row,
